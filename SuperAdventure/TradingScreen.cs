...
namespace SuperAdventure
{
    public partial class TradingScreen : Form
    {
        ...
        {
            // The 5th column (ColumnIndex = 4) is the column with the button. So, if the player clicked the button column, we will sell an item from that row.
            if (e.ColumnIndex == 4)
            {
               
                var itemID = dgvMyItems.Rows[e.RowIndex].Cells[0].Value; // This gets the ID value of the item, from the hidden 1st column                
                Item itemBeingSold = World.ItemByID(Convert.ToInt32(itemID));// Get the Item object for the selected item row
                ...
                {                    
                    _currentPlayer.RemoveItemFromInventory(itemBeingSold);// Remove one of these items from the player's inventory                    
                    _currentPlayer.Gold += itemBeingSold.Price;// Give the player the gold for the item being sold.
                }
            }
        }
        private void dgvVendorItems_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 3)
            {                
                var itemID = dgvVendorItems.Rows[e.RowIndex].Cells[0].Value;// This gets the ID value of the item, from the hidden 1st column                
                Item itemBeingBought = World.ItemByID(Convert.ToInt32(itemID));// Get the Item object for the selected item row
                
                if (_currentPlayer.Gold >= itemBeingBought.Price)// Check if the player has enough gold to buy the item
                {                    
                    _currentPlayer.AddItemToInventory(itemBeingBought);// Add one of the items to the player's inventory                    
                    _currentPlayer.Gold -= itemBeingBought.Price;// Remove the gold to pay for the item
                }
                else
                {
