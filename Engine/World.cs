...
namespace Engine
{
    ...
    {
        /*
            Static list variables. These work similar to the properties in a class. We'll  populate them with all the things in our game world, 
       ...
       */
        ...
        /*
         ...
         With the constants, we can say, get me a monster where the ID equals MONSTER_ID_GIANT_SPIDER.
             */
        //物品：生锈的剑，老鼠尾巴，一块皮毛，蛇的毒牙，蛇皮，棍棒，治疗药水，蜘蛛毒牙，蜘蛛丝，关卡钥匙？

        //Now, it is very clear that we don't want the player to be able to sell adventurer passes. We  won't ever be confused by a magic number 
        //– a hard-coded value that has a special meaning.
        ...
        /*
         This is the static constructor. You might be thinking, "Wait! We can't instantiate a static class, so why does it have a constructor? After all, that's what a constructor is used for instantiating an object!"
        With a static class, the constructor code is run the first time someone uses anything in the class. So, when we start the game and want to display information about the player's current location, 
        and we try to get that data from the World class, the constructor method will be run, and the lists will get populated.
             */
        static World()
        {
            PopulateItems();
        ...
        private static void PopulateItems()
        {
            ...
            //For the Adventurer pass, we set the item's value to -1. We're going to use that as a flag(indicator) value.In our code to display the player's items, we 
            //won't include any items that have a value of -1.Those will be unsellable items.
            ////This is a code smell – a sign that the source code may have a problem.            
            Items.Add(new Item(ITEM_ID_ADVENTURER_PASS, "Adventurer pass", "Adventurer passes", UNSELLABLE_ITEM_PRICE));////Items.Add(new Item(ITEM_ID_ADVENTURER_PASS,"Adventurer pass", "Adventurer passes", -1));
        }
