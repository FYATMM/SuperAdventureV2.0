...
namespace Engine
{
    public static class PlayerDataMapper
    {
        ...
        ////private static readonly string _connectionString = "Data Source=(local);Initial Catalog=SuperAdventure;User Id = sa; Password = 123456";
        public static Player CreateFromDatabase()
        {
            try
            {
                ...
                {
                    connection.Open();// Open the connection, so we can perform SQL commands
                    Player player;

                    using (SqlCommand savedGameCommand = connection.CreateCommand())// Create a SQL command object, that uses the connection to our database. The SqlCommand object is where we create our  SQL statement.
                    {
                        savedGameCommand.CommandType = CommandType.Text;
                        savedGameCommand.CommandText = "SELECT TOP 1 * FROM SavedGame";// This SQL statement reads the first rows in the SavedGame table.For this program, we should only ever have one row, but this will ensure we only get one record in our SQL query results.                                                                                   
                        SqlDataReader reader = savedGameCommand.ExecuteReader(); // Use ExecuteReader when you expect the query to return a row, or rows                        
                        if (!reader.HasRows)// Check if the query did not return a row/record of data
                        {
                            return null;// There is no data in the SavedGame table, so return null (no saved player data)
                        }
                        reader.Read();// Get the row/record from the data reader
                        ...
                        player = Player.CreatePlayerFromDatabase(currentHitPoints, maximumHitPoints, gold, experiencePoints, currentLocationID);// Create the Player object, with the saved game values
                    }

                    using (SqlCommand questCommand = connection.CreateCommand())// Read the rows/records from the Quest table, and add them to the player
                    {
                        ...
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                ...
                                PlayerQuest playerQuest = new PlayerQuest(World.QuestByID(questID));// Build the PlayerQuest item, for this row
                                playerQuest.IsCompleted = isCompleted;
                                player.Quests.Add(playerQuest); // Add the PlayerQuest to the player's property
                            }
                        }
                    }

                    using (SqlCommand inventoryCommand = connection.CreateCommand()) // Read the rows/records from the Inventory table, and add them to the player
                    {
                        ...
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                ...
                                player.AddItemToInventory(World.ItemByID(inventoryItemID), quantity); // Add the item to the player's inventory
                            }
                        }
                    }
                    return player; // Now that the player has been built from the database, return it.
                }
            }
            catch (Exception ex)
            {
        ...
        public static void SaveToDatabase(Player player)
        {
            try
            {
                ...
                {
                    connection.Open();// Open the connection, so we can perform SQL commands                    
                    using (SqlCommand existingRowCountCommand = connection.CreateCommand())// Insert/Update data in SavedGame table
                    {
                        existingRowCountCommand.CommandType = CommandType.Text;
                        existingRowCountCommand.CommandText = "SELECT count(*) FROM SavedGame";
                        int existingRowCount = (int)existingRowCountCommand.ExecuteScalar();// Use ExecuteScalar when your query will return one value
                        ...
                        else
                        {
                            ...
                            using (SqlCommand updateSavedGame = connection.CreateCommand())
                            {
                                ...
                                // Pass the values from the player object, to the SQLquery, using parameters
                                ...
                                // Perform the SQL command.Use ExecuteNonQuery, because this query does not return any results.
                    ...
                    // This is easier than trying to add/delete/update each individual rows Delete existing Quest rows
